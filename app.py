...
# --- STEP 4: TOWS MATRIX ---
    ...
    if not st.session_state.auto_generated:
        ...
        # เจเนอเรตกลยุทธ์แยกรายประเด็น
        so_list, wo_list, st_list, wt_list = [], [], [], []
        
        # SO: Strengths + Opportunities
        if all_s:
            for i, s in enumerate(all_s, 1):
                so_list.append(f"{i}. ด้าน {s}: ส่งเสริมและต่อยอดร่วมกับ {o_kw} เพื่อสร้างมูลค่าเพิ่มทางเศรษฐกิจ")
        else:
            so_list.append(f"1. ขยายผลทุนชุมชนเพื่อสร้างรายได้ฐานราก ตาม {o_kw}")
            
        # WO: Weaknesses + Opportunities
        if all_w:
            for i, w in enumerate(all_w, 1):
                wo_list.append(f"{i}. ด้าน {w}: บูรณาการแผนงาน/งบประมาณตาม {o_kw} เพื่อปิดช่องโหว่และยกระดับศักยภาพ")
        else:
            wo_list.append(f"1. พัฒนาทักษะผู้นำชุมชนขับเคลื่อนการแก้ปัญหาตาม {o_kw}")
            
        # ST: Strengths + Threats
        if all_s:
            for i, s in enumerate(all_s, 1):
                st_list.append(f"{i}. ด้าน {s}: ใช้ความเข้มแข็งของประเด็นนี้เป็นภูมิคุ้มกัน เพื่อลดผลกระทบจาก {t_kw}")
        else:
            st_list.append(f"1. สร้างเครือข่ายฮอมฮักเฝ้าระวังภัยพิบัติและเศรษฐกิจเพื่อรับมือ {t_kw}")
            
        # WT: Weaknesses + Threats
        if all_w:
            for i, w in enumerate(all_w, 1):
                wt_list.append(f"{i}. ด้าน {w}: เฝ้าระวังและปรับปรุงรูปแบบการดำเนินงาน เพื่อป้องกันความเสี่ยงทับซ้อนจาก {t_kw}")
        else:
            wt_list.append(f"1. มุ่งเน้นการพึ่งพาตนเองตามหลักปรัชญาของเศรษฐกิจพอเพียง เพื่อเลี่ยง {t_kw}")
