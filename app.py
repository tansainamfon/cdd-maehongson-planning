...
# --- STEP 3: SWOT ANALYSIS ---
    ...
    if not st.session_state.ip_data.empty:
        for index, row in st.session_state.ip_data.iterrows():
            if row['Importance'] >= 3 and row['Performance'] >= 3: strengths.append(row['ประเด็น'])
            elif row['Importance'] >= 3 and row['Performance'] < 3: weaknesses.append(row['ประเด็น'])
...
# --- STEP 4: TOWS MATRIX ---
    ...
    if not st.session_state.auto_generated:
        ...
        s_txt, w_txt = " และ ".join(strengths) if strengths else "ทุนชุมชน", " และ ".join(weaknesses) if weaknesses else "ข้อจำกัด"
...
# --- STEP 5: VISION & KPI ---
    ...
    st.write("แปลงกลยุทธ์ให้เป็นเป้าหมายที่ชัดเจน เพื่อการดำเนินงานที่มุ่งสู่ 'ฮ่องสอน ฮอมฮัก'")
...
# --- STEP 6: ACTION PLAN ---
    ...
    st.write("แปลงกลยุทธ์สู่การปฏิบัติ (Flagship Projects)")
